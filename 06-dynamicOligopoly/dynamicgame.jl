...
Each period, players simultaneously choose $a \in \texttt{actions}$. The flow payoff for player $i$ is
...
To compute the equilibrium and estimate the model, we will work with choice specific value functions as functions of choice-probabilities. Given choice probabilities, $P[i,a,x] = P(aᵢ=a|x)$, define choice specific value functions as the solution to
```math
v^p[i,a_i,x] = \sum_{a_{-i}} P(a_{-i}|x) \begin{pmatrix}
u(i,a_i, a_{-i},x) + \\ +
\beta \sum_{x'} P(x'|a_i, a_{-i}, x) \sum_{a'} P[i,a',x'] (v^p[i, a', x'] + 
\\
+ E\left[\epsilon[a']\,\vert\,v^p[i, a', x'] + \epsilon[a'] \geq v^p[i, \tilde{a}, x'] + \epsilon[\tilde{a}]\right]) \end{pmatrix}
```

Note that the last term in the above expression can be written as a function of choice probabilities and the distribution of $\epsilon$. For the Gumbel distribution, it becomes ([see here for a derivation](https://stats.stackexchange.com/a/192495/1229)):
```math
...
sim = DG.simulate(g, 4000, out[2], burnin=0, x0=1)
...
sd = DG.simulate(g, 2000, out[2], burnin=0, x0=1);
...
manifest_format = "2.0"
project_hash = "a343d1c1d502bcdd565528924f0e048dd63bc6be"
